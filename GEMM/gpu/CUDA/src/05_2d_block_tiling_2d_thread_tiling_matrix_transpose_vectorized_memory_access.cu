...
template <typename T, size_t BLOCK_TILE_SIZE_M, size_t BLOCK_TILE_SIZE_N,
          size_t BLOCK_TILE_SIZE_K, size_t THREAD_TILE_SIZE_M,
          size_t THREAD_TILE_SIZE_N, size_t NUM_THREADS>
    ...
    for (size_t K_block_tile_id{0}; K_block_tile_id < (k + BLOCK_TILE_SIZE_K - 1) / BLOCK_TILE_SIZE_K; K_block_tile_id++) {
        ...
        for (size_t k_block_tile_idx{0}; k_block_tile_idx < BLOCK_TILE_SIZE_K; ++k_block_tile_idx) {
            ...
            for (size_t m_thread_tile_idx{0}; m_thread_tile_idx < VECTORIZED_THREAD_TILE_SIZE_M; ++m_thread_tile_idx) {
                *reinterpret_cast<int4*>(&A_thread_tile[m_thread_tile_idx * NUM_VECTOR_UNITS]) = *reinterpret_cast<int4*>(&A_block_tile[k_block_tile_idx][M_thread_tile_index_start + m_thread_tile_idx * NUM_VECTOR_UNITS]);
            }
                                       ...
                                       T* C, size_t ldc, cudaStream_t stream)
{
    ...
    gemm_v05_vectorized<T, BLOCK_TILE_SIZE_M, BLOCK_TILE_SIZE_N, BLOCK_TILE_SIZE_K, THREAD_TILE_SIZE_M, THREAD_TILE_SIZE_N, NUM_THREADS>
