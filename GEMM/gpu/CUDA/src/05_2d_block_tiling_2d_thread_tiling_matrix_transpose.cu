...
__global__ void gemm_v05(size_t m, size_t n, size_t k, T alpha, T const* A,
    ...
    for (size_t K_block_tile_id{0}; K_block_tile_id < (k + BLOCK_TILE_SIZE_K - 1) / BLOCK_TILE_SIZE_K; K_block_tile_id++) {
        ...
        load_data_from_global_memory_to_shared_memory_transposed<T, BLOCK_TILE_SIZE_M, BLOCK_TILE_SIZE_N, BLOCK_TILE_SIZE_K, NUM_THREADS>(
        ...
        for (size_t k_block_tile_idx{0}; k_block_tile_idx < BLOCK_TILE_SIZE_K; ++k_block_tile_idx) {
            ...
            for (size_t m_thread_tile_idx{0}; m_thread_tile_idx < THREAD_TILE_SIZE_M; ++m_thread_tile_idx) {
                A_thread_tile[m_thread_tile_idx] = A_block_tile[k_block_tile_idx][M_thread_tile_index_start + m_thread_tile_idx];
            }
...
void launch_gemm_kernel_v05(size_t m, size_t n, size_t k, T const* alpha,
    ...
    gemm_v05<T, BLOCK_TILE_SIZE_M, BLOCK_TILE_SIZE_N, BLOCK_TILE_SIZE_K, THREAD_TILE_SIZE_M, THREAD_TILE_SIZE_N, NUM_THREADS>
