...
__global__ void gemm_v02(size_t m, size_t n, size_t k, T alpha, T const* A,
    ...
    unsigned int const c_row_idx{A_block_tile_id * BLOCK_TILE_SIZE_M + threadIdx.y};
    unsigned int const c_col_idx{B_block_tile_id * BLOCK_TILE_SIZE_N + threadIdx.x};
    ...
    while (K_block_tile_id < K_block_tile_num) {
        ...
        unsigned int K_start{K_block_tile_id * BLOCK_TILE_SIZE_K};
        A_block_tile[threadIdx.y][threadIdx.x] = A[c_row_idx * lda + K_start + threadIdx.x];
        B_block_tile[threadIdx.y][threadIdx.x] = B[(K_start + threadIdx.y) * ldb  + c_col_idx];
        K_block_tile_id++;
        __syncthreads();
        // Compute the sum
